# Arcane scaling benchmarks — full experiment report

This document is the long-form **experiment write-up** (motivation, design, detailed tables, discussion). For **how to reproduce** and a short **results summary**, see the [README](../README.md) and [REPRODUCIBILITY.md](../REPRODUCIBILITY.md).
...
This repository is self-contained: it includes the **orchestration scripts** (`scripts/swarm/Run-ArcaneScalingSweep.ps1`, `Run-SpacetimeDBCeilingSweep.ps1`) plus the benchmark swarm code and the SpacetimeDB module source. **Benchmark v2** (`scripts/benchmark/Run-Benchmark-V2.ps1`) runs the same workload in **Docker** with **resource limits**; it can use **published** Arcane images for reproducibility without private submodules. See [BENCHMARK_V2_METHOD.md](BENCHMARK_V2_METHOD.md).
...
## 4. Results (legacy / native profile)

The tables below come from **native-process** runs (`scripts/swarm/`), not from Benchmark v2 Docker caps.
...
Step-by-step instructions: **[REPRODUCIBILITY.md](../REPRODUCIBILITY.md)**. Canonical parameters: **[CANONICAL_PARAMETERS.md](CANONICAL_PARAMETERS.md)**.
