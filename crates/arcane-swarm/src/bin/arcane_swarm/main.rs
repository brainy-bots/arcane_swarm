...
use arcane_swarm::{
    parse_args, run_reporter, ArcaneEndpoint, Backend, Metrics, ReporterConfig, SwarmMode,
    VISIBILITY_RADIUS,
};
...
async fn main() {
    ...
    if cfg.run_forever || cfg.control_port > 0 {
        control::run_control_mode(cfg, tick_interval).await;
        return;
    }
    ...
    for i in 0..cfg.players {
        ...
        let handles_bundle = runtime::SharedHandles::from_player_loop_shared(&loop_shared);
        handles.push(backend_runtime.spawn_player(handles_bundle.clone(), params.clone()));
        let _ = backend_runtime.spawn_read(&handles_bundle, &params, cfg.read_rate);
    }
